...
import {JULY,canListen} from './music-catalogue.js';
...
export function buildOpenings(rows=[],user={}){
  ...
  const items=[
    ...
    ...JULY.tracks.map(track=>({id:'music-'+track.slug,title:track.title,description:'18 juillet 2019 · musique et paroles',level:track.minLevel,open:canListen(track,access),href:'/musique#track-'+track.slug,lyricsHref:'/chanson/'+track.slug})),
  ...
  return {author:access.admin,nextLevel,items:items.filter(item=>access.admin||item.open||item.level<=nextLevel).map(item=>{
