...
const state = {
  ...
  sel: null, // sélection : {type:'song'|'line'|'word', lineId, start, end}
};
      ...
      <ol class="song-list">
        ${al.songs.map((s) => `
          <li>
            ...
            <span class="song-meta">${s.line_count ? `${s.annotation_count} explication${s.annotation_count > 1 ? 's' : ''}` : 'paroles à venir'}</span>
          </li>`).join('')}
      </ol>
    </section>`).join('');
...
function lineHasNote(lineId) {
  return state.song.annotations.some((a) => a.line_id === lineId && a.word_start == null);
}

function wordHasNote(lineId, idx) {
  return state.song.annotations.some(
    (a) => a.line_id === lineId && a.word_start != null && idx >= a.word_start && idx <= a.word_end
  );
}
...
function renderSongPage() {
    ...
    ? `<div class="lyrics">${lines.map((line) => {
        ...
        return `<div class="${lineClasses.join(' ')}" data-line-id="${line.id}">
          ...
          <button class="line-note-btn ${isSelLine && sel.type === 'line' ? 'active' : ''}"
                  data-line-btn="${line.id}" title="Expliquer cette phrase">&#128172;</button>
        </div>`;
      }).join('')}</div>
      <p class="hint">Cliquez sur un mot pour l’expliquer (maintenez <strong>Maj</strong> pour sélectionner plusieurs mots
      d’une même phrase), ou sur &#128172; pour expliquer la phrase entière.</p>`
  ...
  app.innerHTML = `
    ...
    ${song.youtube_url ? `<p><a href="${esc(song.youtube_url)}" target="_blank" rel="noopener">▶ Écouter sur YouTube</a></p>` : ''}
...
/* --------- panneau latéral : explications de la sélection en cours ------- */
...
function annotationForm(id, placeholder, buttonLabel) {
  if (!state.user) {
    return `<p class="empty-note"><a href="/connexion" data-link>Connectez-vous</a> pour proposer une explication.</p>`;
  }
...
function bindAnnotationActions(container) {
  container.querySelectorAll('[data-del]').forEach((btn) => {
    btn.onclick = async () => {
      if (!confirm('Supprimer cette explication ?')) return;
      try {
...
function renderPanel() {
  ...
  if (sel) {
    ...
    if (sel.type === 'word') {
      ...
      const wordAnns = annotationsFor(
        (a) => a.line_id === sel.lineId && a.word_start != null &&
      ...
      html += `<div class="panel-card">
        ...
        )).join('') || '<p class="empty-note">Aucune explication pour l’instant.</p>'}
        ${annotationForm('word-ann-form', `Que signifie « ${quote} » ici ?`, 'Expliquer ces mots')}
      </div>`;
    }

    const lineAnns = annotationsFor((a) => a.line_id === sel.lineId && a.word_start == null);
    html += `<div class="panel-card">
      <h3>La phrase</h3>
      <div class="panel-target">« ${esc(line ? line.text : '')} »</div>
      ${lineAnns.map((a) => annotationCard(a)).join('') || '<p class="empty-note">Aucune explication pour l’instant.</p>'}
      ${annotationForm('line-ann-form', 'Que signifie cette phrase ?', 'Expliquer cette phrase')}
    </div>
    <button class="link-btn" id="clear-sel">← Revenir à la chanson</button>`;
  } else {
    const songAnns = annotationsFor((a) => a.line_id == null);
    html += `<div class="panel-card">
      <h3>À propos de la chanson</h3>
      ${songAnns.map((a) => annotationCard(a)).join('') || '<p class="empty-note">Aucune explication générale pour l’instant.</p>'}
      ${annotationForm('song-ann-form', `Le sens général de « ${song.title} »…`, 'Expliquer la chanson')}
    </div>`;
  }
  ...
  if (sel) {
    if (sel.type === 'word') {
      bindAnnotationForm('word-ann-form', {
        song_id: song.id, line_id: sel.lineId, word_start: sel.start, word_end: sel.end,
      });
    }
    bindAnnotationForm('line-ann-form', { song_id: song.id, line_id: sel.lineId });
    const clear = document.getElementById('clear-sel');
    if (clear) clear.onclick = () => { state.sel = null; renderSongPage(); };
  } else {
    bindAnnotationForm('song-ann-form', { song_id: song.id });
  }
...
async function pageAdmin() {
    ...
    <div class="admin-grid">
      ...
      <section class="admin-card lyrics-editor" style="grid-column: 1 / -1">
        ...
        <form id="lyrics-form" hidden>
          ...
          <p class="hint">⚠ Remplacer le texte supprime les explications déjà attachées aux anciennes phrases et mots.</p>
          <div class="error-msg"></div>
  ...
  app.querySelectorAll('[data-del-song]').forEach((btn) => {
    btn.onclick = async () => {
      if (!confirm('Supprimer cette chanson, ses paroles et toutes ses explications ?')) return;
  ...
  lyricsSelect.onchange = async () => {
    ...
    if (!opt || !opt.value) { lyricsForm.hidden = true; return; }
