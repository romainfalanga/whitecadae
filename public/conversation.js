window.WCConversation={async page(){
  ...
  app.innerHTML=`<section class="conversation-page"><header class="conv-heading"><div><h1>Conversation</h1><p class="subtitle">Croiser les lectures, partager des pistes et donner une forme à ses idées.</p></div><a href="/parcours?view=ouvertures" data-link class="conv-access">Mes paliers et mes accès →</a></header>
    <section class="conv-filter-panel" aria-label="Filtrer la conversation"><div class="conv-filter-heading"><h2>Explorer les échanges</h2><button type="button" id="conv-reset" hidden>Tout réinitialiser</button></div>
      ...
      <button type="button" class="conv-level-toggle" id="conv-level-toggle" aria-expanded="false" aria-controls="conv-level-controls"><span>Échelons <strong id="conv-level-label"></strong></span><span class="conv-chevron" aria-hidden="true">⌄</span></button>
      <div class="conv-level-controls" id="conv-level-controls" hidden><label for="conv-f-mode">Quels messages afficher ?<select id="conv-f-mode"><option value="tout">Tous mes accès</option><option value="max">Jusqu’à un échelon</option><option value="exact">Un échelon précis</option><option value="min">À partir d’un échelon</option><option value="historique">Messages historiques</option></select></label><label id="conv-level-field" for="conv-f-niveau">Échelon<select id="conv-f-niveau">${options(filter.niveau)}</select></label><p class="conv-help">Ces filtres affichent uniquement les messages auxquels tu as accès.</p></div>
    </section><div class="conv-selection" aria-live="polite"><p id="conv-selection-label"></p><span id="conv-count"></span></div>
  ...
  function showFilters(){
    ...
    el('f-mode').value=filter.mode;el('f-niveau').value=filter.niveau;el('level-field').hidden=['tout','historique'].includes(filter.mode);
    el('level-label').textContent=levelLabel();el('selection-label').textContent=(themeBy(filter.theme)?.label||'Tous les thèmes')+' · '+levelLabel();
    el('reset').hidden=filter.theme==='tout'&&filter.mode==='tout';
  }
  function render(placement='keep'){
    ...
    el('count').textContent=`${data.messages.length} message${data.messages.length>1?'s':''} affiché${data.messages.length>1?'s':''}`;
  ...
  const expand=show=>{el('level-controls').hidden=!show;el('level-toggle').setAttribute('aria-expanded',String(show));};
  el('level-toggle').onclick=()=>expand(el('level-controls').hidden);
  expand(filter.mode!=='tout');
  ...
  el('reset').onclick=()=>{Object.assign(filter,{theme:'tout',mode:'tout',niveau:'2'});expand(false);el('level-toggle').focus();apply();};
