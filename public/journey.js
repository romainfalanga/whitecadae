window.WCJourney=(()=>{
  ...
  function openingsHtml(data){
    ...
    const levels=Array.from({length:Math.max(data.echelon,...items.map(item=>item.level))+1},(_,level)=>level);
    ...
    const current=items.filter(item=>item.open&&item.level===data.echelon);
    return `<div class="openings-intro"><h2>Mes paliers et mes accès</h2><p>${author?'Vue auteur : tous les paliers de contenu prévus sont affichés pour préparer la progression des joueurs.':'Retrouve chaque échelon parcouru, ce qu’il t’a ouvert et la prochaine étape. Tes accès précédents restent disponibles.'}</p>
      <div class="opening-position"><div><span>Tu es ici</span><strong>Échelon ${data.echelon}</strong><p>${current.length?'Ouvert à ce palier : '+current.map(item=>esc(item.title)).join(' · '):'Tu conserves tous tes accès acquis.'}</p></div><button type="button" id="opening-current">Voir mon palier ↓</button></div>${nextLevel!==null?`<p class="opening-next">Prochaine ouverture à l’échelon <strong>${nextLevel}</strong> · encore ${nextLevel-data.echelon} échelon${nextLevel-data.echelon>1?'s':''}</p>`:''}</div>
      <ol class="opening-timeline">${levels.map(level=>`<li id="palier-${level}" class="opening-step ${data.echelon>=level?'is-reached':''} ${data.echelon===level?'is-current':''}" ${data.echelon===level?'aria-current="step"':''}><div class="opening-milestone"><span>${level===0?'Départ':'Échelon'}</span><strong>${level}</strong>${data.echelon===level?'<small>Tu es ici</small>':''}</div><div class="opening-cards">${items.filter(item=>item.level===level).map(card).join('')||`<p class="opening-empty">${data.echelon>=level?'Pas de nouvel espace à ce palier. Tes accès précédents restent ouverts.':'Pas d’ouverture de contenu prévue à ce palier.'}</p>`}</div></li>`).join('')}</ol>${clock?`<section class="opening-discovery"><h2>Ouvert par une découverte</h2><p>Certains chemins demandent un signe précis, quel que soit ton échelon.</p>${card({title:'Horloge',description:'La réponse Horloge dans Aiguille donne accès au laboratoire des durées.',open:true,href:clock.href})}</section>`:''}<p class="journey-hint">Ouvrir un espace ou un morceau ne rapporte pas de point. Chaque réponse complète distincte fait gagner un échelon.${author?' Ces paliers décrivent les contenus actuellement prévus, pas la fin du jeu.':''}</p>`;
  }
  async function page(){
    ...
    document.getElementById('opening-current').onclick=()=>document.getElementById('palier-'+data.echelon)?.scrollIntoView({block:'start',behavior:'instant'});
