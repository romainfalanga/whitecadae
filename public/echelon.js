...
window.WCGame=(()=>{
  ...
  function render(){
    ...
    app.innerHTML=`<section class="eg-page">${header()}<div id="eg-feedback" class="eg-feedback" role="status" aria-live="polite"></div>${data.anonyme?login():'<a class="eg-progress-link" href="/parcours?view=ouvertures" data-link>Mon palier et mes accès →</a>'}${isClock()?`<div class="eg-durations">${WC57.tracks.map(t=>`<div><span>${e(t.title)}</span><strong>${mmss(Math.floor(t.duration))}</strong><button class="eg-listen" data-play-track="${e(t.slug)}">Écouter</button></div>`).join('')}</div>`:''}<div id="eg-game-content" class="${isClock()?'eg-labs':'eg-entries'}"></div><footer class="eg-footer"><a href="/musique#album-57" data-link>Écouter 57</a><a href="/paroles" data-link>Lire les paroles</a>${isClock()?'<a href="/echelon" data-link>Retour à Échelons</a>':''}</footer></section>`;
  ...
  async function submit(p,payload,button){
    ...
    try{
      ...
      feedback(result.gained?`+${result.gained} ${result.gained>1?'échelons':'échelon'}.${newlyVisible?' Une nouvelle énigme apparaît.':''}`:result.message||'Une partie de ce signe est trouvée.',p.id,result.opened||[]);
