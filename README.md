...
La vue **Mes paliers** (`/parcours?view=ouvertures`) montre les contenus accessibles et le prochain palier : les pages de départ, Conversation à 2, les trois morceaux et leurs paroles à 5/6/7, puis Vidéographie à 9 et les morceaux de Fais Mieux à 10/12/14. Chaque joueur voit aussi les paliers intermédiaires, sa position actuelle et le détail des accès gagnés. Les accès historiques conservés sont signalés. Horloge apparaît séparément quand la découverte l’a ouvert. Le compte administrateur voit tous les paliers de contenu, y compris les échelons intermédiaires sans ouverture, pour organiser la progression. Cette vue ne révèle aucune énigme future et ne crée aucun point. Ses permissions proviennent de `src/content-access.js` et des mêmes seuils musicaux que les API. Le plan et les règles de conservation sont documentés dans `docs/parcours-aa-conversation.md`.
